...
import { evaluatePublicationPolicy } from './publication-policies.mjs';

const path = process.argv[2];
if (!path) throw new Error('usage: node benchmark-publication-overhead.mjs <direct-agentdojo-json> [repetitions]');
...
// Warm the JIT and module-level caches before recording samples.
for (let warmup = 0; warmup < 20; warmup += 1) {
  for (const item of traces) {
    const decision = evaluatePublicationPolicy(item.contract, item.trace, 'full');
...
const result = {
  benchmark: 'publication CPU microbenchmark',
  scope: 'authorization evaluation on frozen 60-task reference traces; excludes provider/network/model latency',
  environment: {
