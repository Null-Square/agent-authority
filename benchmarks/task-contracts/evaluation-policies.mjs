...
function dynamicAllowed(contract, trace, index, event, projected, mode) {
  ...
  for (const [field, spec] of Object.entries(rule.dynamic || {})) {
    ...
    if (mode === 'provenance' || mode === 'request-provenance') {
      if (hasSelectorMatch(spec)) {
        if (!priorEvidenceMatch(trace, index, value, spec.sourceActions, { allowRequest: mode === 'request-provenance' })) {
          return { allowed: false, reason: { index, code: 'eval_provenance_mismatch', action: event.action, field } };
        }
        continue;
      }
      ...
      return { allowed: false, reason: { index, code: 'eval_evidence_mismatch', action: event.action, field } };
    }
    ...
    if (!inferred) return { allowed: false, reason: { index, code: 'eval_strict_evidence_mismatch', action: event.action, field } };
  }
  return { allowed: true };
}

export const EVALUATION_POLICIES = Object.freeze({
  full: {
...
export function evaluatePolicy(contract, trace, policyName = 'full') {
  const policy = EVALUATION_POLICIES[policyName];
  if (!policy) throw new Error(`unknown evaluation policy: ${policyName}`);
