...
if (!path) throw new Error('usage: node generate-evaluation-matrix.mjs <direct-agentdojo-json>');
...
function replacementFor(value, salt = 1) {
  ...
  if (typeof value === 'string') {
    if (value.includes('@')) {
      ...
      return `${local}+eval${salt}@${domain}`;
    }
    return `eval-${salt}-${value}`;
  }
...
function freshIdentityValue(value, salt) {
  ...
  for (let index = 0; index < oldIds.length; index += 1) {
    ...
    const newId = typeof oldId === 'number' ? oldId + 12000 + salt + index : `${oldId}-eval-${salt + index}`;
...
function makeEvidenceVariant(source, contract, binding, salt) {
  ...
  if (binding.match.startsWith('selector-')) {
    ...
    const newValue = `${prefix}Eval${salt}`;
...
function generateRequestSelfAuthorizationMutants(source, contract) {
  ...
  for (const binding of contract.metadata.bindings) {
    ...
    const trace = clone(base);
    trace[binding.sourceIndex].args = { ...(trace[binding.sourceIndex].args || {}), eval_probe: clone(evil) };
...
const result = {
  ...
  note: 'Internal reproducible comparators with explicit semantics; external systems are not approximated or relabeled.',
...
for (const source of direct.tasks) {
  ...
  const taskResult = {
    ...
    selectorBindings: contract.metadata.bindings.filter((binding) => String(binding.match).startsWith('selector-')).length
  };
  ...
  for (const item of cases) {
    ...
    for (const policyName of policyNames) {
      const allowed = evaluatePolicy(contract, item.trace, policyName).allowed;
