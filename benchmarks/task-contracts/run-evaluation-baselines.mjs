...
if (!cohortPath) throw new Error('usage: node run-evaluation-baselines.mjs <direct-agentdojo-json>');

const run = spawnSync(process.execPath, [
  new URL('./generate-evaluation-matrix.mjs', import.meta.url).pathname,
  cohortPath
], { encoding: 'utf8' });

if (run.status !== 0) {
  process.stderr.write(run.stderr || 'evaluation matrix generator failed\n');
...
const result = {
  schemaVersion: 2,
  tasks: raw.tasks,
  note: 'The primary matrix excludes the legacy raw transplant generator because it contains five known invalid/over-broad cases. Exact transplant behavior is validated separately by run-exact-transplant-audit.mjs and the provider-boundary gate. Raw counts remain as diagnostics.',
  generated: {
    ...raw.generated,
    primaryAttacks: full.attacks,
  ...
  gates: {
    ...
    fullBlocksAllPrimaryAttacks: full.blockedAttacks === full.attacks,
