...
export function validateMission(mission) {
  ...
  if (!Array.isArray(mission.resources) || mission.resources.length === 0) errors.push('resources must contain at least one service');
...
export function evaluateMissionPolicy(missionInput, request, now = new Date()) {
  ...
  if (expiresAt && now >= new Date(expiresAt)) return deny('mission_expired', `mission expired at ${expiresAt}`);
  ...
  if (budget && context.amount !== undefined) {
    ...
    if (Number(context.amount) > Number(budget.amount)) return deny('budget_exceeded', `requested amount ${context.amount} exceeds mission cap ${budget.amount}`);
  }

  const resource = mission.resources.find((item) => matchPattern(item.service, service));
  if (!resource) return deny('service_not_authorized', `${service} is outside this mission`);
  if (anyMatch(resource.deny || [], action)) return deny('explicit_deny', `${service}:${action} is explicitly denied`);
  if (!anyMatch(resource.allow || [], action)) return deny('action_not_authorized', `${service}:${action} is not allowed by this mission`);
  ...
  if (approval) return { decision: 'require_approval', reason: approval.reason || 'human approval required by mission policy', rule: approval };
  return { decision: 'allow', reason: 'authorized by mission policy' };
}
...
export function deriveMission(parentInput, childSpec) {
  ...
  for (const childResource of childSpec.resources) {
    const parentResource = parent.resources.find((r) => matchPattern(r.service, childResource.service));
    if (!parentResource) throw new Error(`child service ${childResource.service} is not authorized by parent`);
    for (const childAllow of childResource.allow || []) {
      if ((parentResource.deny || []).some((d) => matchPattern(d, childAllow))) throw new Error(`child capability ${childAllow} expands parent authority`);
      if (!(parentResource.allow || []).some((a) => matchPattern(a, childAllow))) throw new Error(`child capability ${childAllow} is not authorized by parent`);
    }
  }
  ...
  return assertMission({
    ...
    constraints: {
      ...
      max_delegation_depth: Math.min(childSpec.constraints?.max_delegation_depth ?? 0, Math.max(0, maxDepth - parentDepth - 1))
    },
